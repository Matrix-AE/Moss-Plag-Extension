...
const { sendMagicCodeEmail } = require("./resend-mail");
...
function createPasswordAuthService({
  ...
  const otps = new Map(); // nonce -> { email, codeHash, purpose, expiresAt, used }
  const sessions = new Map(); // accessToken -> session
  const refreshByHash = new Map();
  const retiredRefresh = new Set();
  ...
  function register({ email, password }) {
    ...
    if (findUser(normalized)) return { ok: false, error: "email-taken", status: 409 };
  ...
  function login({ email, password, deviceId }) {
    ...
    return startOtp({
      email: normalized,
      purpose: user.emailVerified ? "login" : "register",
    });
  }
  ...
  async function startOtp({ email, purpose }) {
    ...
    if (!mailed.ok) {
      ...
      return {
        ok: false,
        error: "otp-email-failed",
        status: 502,
        detail: mailed.error || mailed.detail || "send-failed",
      };
    }
    ...
    if (!production && process.env.AUTH_RETURN_OTP === "1") {
      result.devCode = code;
    }
    return result;
  }

  function verifyOtp({ nonce, code, deviceId }) {
    if (!nonce || !code || !deviceId) {
  ...
  function authenticateProvider({ provider, subject, email, emailVerified, deviceId }) {
    const normalized = normalizeEmail(email);
    if (!["google", "microsoft"].includes(provider)) {
      return { ok: false, error: "provider-not-supported", status: 400 };
    }
    if (!subject || !deviceId || !isEmail(normalized) || !emailVerified) {
      return { ok: false, error: "provider-identity-invalid", status: 401 };
    }
  ...
  function issueSession({ user, deviceId }) {
  ...
  function refresh({ refreshToken, deviceId }) {
    ...
    if (retiredRefresh.has(h)) {
      const prior = [...sessions.values()].find((s) => s.deviceId === deviceId);
      if (prior) logoutAll(prior.userId);
      return { ok: false, error: "refresh-reuse", status: 401 };
    }
    const session = refreshByHash.get(h);
    if (!session) return { ok: false, error: "unknown-refresh", status: 401 };
    if (session.revoked) return { ok: false, error: "revoked", status: 401 };
    if (session.refreshExpiresAt < now()) return { ok: false, error: "refresh-expired", status: 401 };
    if (session.deviceId !== deviceId) return { ok: false, error: "device-binding", status: 401 };
  ...
  function authorize({ accessToken }) {
    ...
    if (!session || session.revoked) return { ok: false, error: "unauthorized", status: 401 };
    if (session.accessExpiresAt < now()) return { ok: false, error: "access-expired", status: 401 };
    return {
      ok: true,
...
function validatePassword(password) {
  ...
  const rules = {
    length: value.length >= MIN_PASSWORD_LEN,
  ...
  return {
    ...
    failedRules: Object.entries(rules)
...
function saveStore(storePath, state) {
